...
class Star {
  constructor(z) {
    ...
    this.radius = map(z, 0, 100, 1, 10); // calculating radius based on the z index. The bigger the z index, the smaller the radius
    ...
    this.orbitRadius = random(-10, 10);
    this.lerpNumber = random(0.1, 0.25);
  }
  ...
  engorge() {
    if (this.mouseOver() && this.radius < 30) {
      this.radius++;
      this.pos.z = 0;
    }
    for (let i = 0; i < this.constellation.length; i++) {
      ...
      this.constellation[i].pos.z = 0.1;
